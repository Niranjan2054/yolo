...
def arg_parse():
    ...
    parser.add_argument('--det',dest='det',help="Image/ Diectory to store detection to",default='det',type=str)
    ...
    parser.add_argument('--weights',dest="weightsfile",help="Weights File",default='yolov3.weights',type=str)
...
#Py Torch Variables for images
im_batches = list(map(prep_image,loaded_ims,[inp_dim for x in range(len(imlist))]))


#list containing dimension of original images
im_dim_list = [(x.shape[1],x.shape[0]) for x in loaded_ims] # 1 belongs to width of image and 0 belong to row i.e. height of the image
...
for i,batch in enumerate(im_batches):
    ...
    if not write:
        ...
        output = torch.cat((ouput,prediction))
